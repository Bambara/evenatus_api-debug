  ...
  try {
    ...
    const smsReq = https.request(options, (smsRes) => {
      ...
      smsRes.on("end", () => {
        ...
        res.status(404).json({ warning: "No more data in response." });
      });
    });
...
const catchNewSubscriptionNotify = asyncHandler(async (req, res) => {
  try {
    res.status(200).json({ message: "Subscription ද ඔ්නේ" });
  } catch (e) {
    console.error(chalk.red(e.toString()));
    res.status(500).json({ error: e.toString() });
  }
...
const directDebit = asyncHandler(async (req, res) => {
  try {
    const postData = {
      applicationId: process.env.MSPACE_APP_ID,
      password: process.env.MSPACE_APP_PASS,
      externalTrxId: req["body"].trxId,
      subscriberId: req["body"].payerId,
